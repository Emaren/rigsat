...
List<TableRow> createTableRows(
  ...
  for (int i = 0; i < payPeriodDates.length; i++) {
    ...
    if (data != null) {
      hoursWorked = (data.endHour + data.endMinute / 60.0) -
          (data.startHour + data.startMinute / 60.0) -
          (data.lunchTaken ? 0.5 : 0);
      print(
          'Data for date ${data.dateYMD}: startHour=${data.startHour}, startMinute=${data.startMinute}, endHour=${data.endHour}, endMinute=${data.endMinute}, lunchTaken=${data.lunchTaken}, hoursWorked=$hoursWorked');
    ...
    tableRows.add(TableRow(
      children: [
        ...
        Padding(
          ...
          child: Center(
            child: AutoSizeText(
              data != null ? data.dateYMD.substring(8) : '-',
              minFontSize: 2,
              maxLines: 1,
              textAlign: TextAlign.center,
            ),
